...
function init() {
  ...
  gameObj.ctxRader = raderCanvas.getContext('2d'); // ランキング用のキャンバス
  ...
  gameObj.ctxScore = scoreCanvas.getContext('2d'); // 潜水艦の画像

  var submarineImage = new Image();
  submarineImage.src = '/images/submarine.png';
  gameObj.submarineImage = submarineImage; // ミサイルの画像

  gameObj.missileImage = new Image();
  gameObj.missileImage.src = '/images/missile.png'; // 爆発の画像集
...
function drawRanking(ctxScore, playersMap) {
  ...
  for (var i = 0; i < 10; i++) {
    ...
    var rank = i + 1;
    ctxScore.fillText("".concat(rank, "th ").concat(playersArray[i][1].displayName, " ").concat(playersArray[i][1].score), 10, 220 + rank * 26);
  }
}
...
socket.on('map data', function (compressed) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      gameObj.playersMap.set(player.playerId, player); // 自分の情報も更新
...
function drawMap(gameObj) {
  ...
  drawObj(gameObj.airMap, 0, 220, 255); // 飛んでいるミサイルの描画
...
function calculationBetweenTwoPoints(pX, pY, oX, oY, gameWidth, gameHeight, raderCanvasWidth, raderCanvasHeight) {
  ...
  if (pX <= oX) {
    // 右から
    distanceX = oX - pX;
    drawX = raderCanvasWidth / 2 + distanceX; // 左から
  ...
  if (pY <= oY) {
    // 下から
    distanceY = oY - pY;
    drawY = raderCanvasHeight / 2 + distanceY; // 上から
    ...
    // 上から
    distanceY = pY - oY;
    drawY = raderCanvasHeight / 2 - distanceY; // 下から
